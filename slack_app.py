...
def _process(loom_url: str, channel: str, thread_ts: str) -> None:
    _post(channel, thread_ts, f"⏳ Got it! Starting pipeline for:\n`{loom_url}`")
    try:
        from src.pipeline import run_pipeline
        result = run_pipeline(loom_url)
        _post(
            channel,
            thread_ts,
            f"✅ Done! *{result['title']}*\n{result['vimeo_url']}",
        ...
        _post(channel, thread_ts, f"❌ Pipeline failed: `{exc}`")
...
def slack_events():
    ...
    if data.get("type") == "event_callback":
        ...
        if is_user_message:
            match = LOOM_RE.search(event.get("text", ""))
            if match:
                loom_url = match.group()
                channel = event["channel"]
                thread_ts = event.get("thread_ts") or event["ts"]
                threading.Thread(
                    target=_process,
                    args=(loom_url, channel, thread_ts),
                    daemon=True,
                ).start()
